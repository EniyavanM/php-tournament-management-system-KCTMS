...
<head>
    ...
    if(isset($_POST["upd_item"])){
        ...
        if($cartupdate_result){
            header("location: Tournment_page.php?up_crt=1");
        }else{
            header("location: Tournament_page.php?up_crt=0");
        }
        exit(1);
    }
    ...
    <title>matcheshes Item | kctms</title>
</head>
...
<body class="d-flex flex-column h-100">
    <?php 
        ...
        $s_id = $_GET["s_id"];
        $f_id = $_GET["f_id"];
        $query = "SELECT * FROM matcheshes WHERE s_id = {$s_id} AND f_id = {$f_id} LIMIT 0,1";
    ...
    <div class="row my-4">
        ...
        <div class="row row-cols-1 row-cols-md-2 mb-5">
            <div class="col mb-3 mb-md-0">
                <img 
                    <?php
                        if(is_null($matcheshes_row["f_pic"])){echo "src='img/default.png'";}
                        else{echo "src=\"img/{$matcheshes_row['f_pic']}\"";}
                    ?> 
                    class="img-fluid rounded-25 float-start" 
                    alt="<?php echo $matcheshes_row["f_name"]?>">
            </div>
            <div class="col text-wrap">
                <h1 class="fw-light"><?php echo $matcheshes_row["f_name"]?></h1>
                ...
                <div class="form-amount">
                    <form class="mt-3" method="POST" action="ply_update_cart.php">
