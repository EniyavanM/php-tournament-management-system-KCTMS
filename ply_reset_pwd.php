...
<head>
    <?php 
        ...
        if(isset($_POST["rst_confirm"])){
            ...
            if($newpwd==$newcfpwd){
                $query = "UPDATE player SET c_pwd = '$newpwd' WHERE c_id=$cust_id";
                $result = $mysqli -> query($query);
                if($result) {
                    header("location: ply_reset_success.php");
                }else{
                    header("location: ply_reset_fail.php?err={$mysqli->errno}");
                }
                exit();
            }else{
            ?>
        ...
        $query = "SELECT c_firstname,c_lastname,c_id FROM player WHERE c_username = '$cust_un' AND c_email = '$cust_em' LIMIT 0,1";
        $result = $mysqli -> query($query);
    ...
    <title>Reset Password | kctms</title>
</head>
...
<body class="d-flex flex-column h-100">
    <header class="navbar navbar-light fixed-top bg-light shadow-sm mb-auto">
        <div class="container-fluid mx-4">
            <a href="index.php">
                <img src="img/Color logo - no background.png" width="125" class="me-2" alt="kctms Logo">
            </a>
        </div>
    </header>
    ...
    <div class="container form-signin mt-auto">
        ...
        <form method="POST" action="ply_reset_pwd.php" class="form-floating">
