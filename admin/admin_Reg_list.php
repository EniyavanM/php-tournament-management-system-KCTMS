...
<body class="d-flex flex-column h-100">
        ...
        <div class="mt-4 border-bottom">
            ...
            <div class="row row-cols-1 notibar">
                <div class="col mt-2 ms-2 p-2 bg-success text-white rounded text-start">
                    ...
                    <span class="ms-2 mt-2">Successfully updated Registration.</span>
            ...
            <div class="row row-cols-1 notibar">
                <div class="col mt-2 ms-2 p-2 bg-danger text-white rounded text-start">
                    <i class="bi bi-x-circle ms-2"></i><span class="ms-2 mt-2">Failed to update registration status.</span>
                    <span class="me-2 float-end"><a class="text-decoration-none link-light" href="admin_ply_list.php">X</a></span>
                </div>
            </div>
            ...
            <h2 class="pt-3 display-6">Registration List</h2>
            <form class="form-floating mb-3" method="GET" action="admin_Reg_list.php">
                <div class="row g-2">
                    <div class="col">
                   ...
                   <!-- <div class="col">
                    ...
                    <div class="col">
                        <select class="form-select" id="s_id" name="s_id">
                            <option selected value="">Tournament Name</option>
                            <?php
                                $option_query = "SELECT DISTINCT s.s_id, s.s_name
                                FROM mreg_status orh INNER JOIN tournament s ON orh.s_id = s.s_id;";
                                $option_result = $mysqli -> query($option_query);
                                $opt_row = $option_result -> num_rows;
                                if($option_result -> num_rows != 0){
                                    while($option_arr = $option_result -> fetch_array()){
                    ...
                    <div class="col">
                        <select class="form-select" id="orderstatus" name="os">
                            <?php if(isset($_GET["search"])){?>
                            <option selected value="">REG Status</option>
                            <option value="VRFY" <?php if($_GET["os"]=="VRFY"){ echo "selected";}?>>ReG Verifying</option>
                            <option value="ACPT" <?php if($_GET["os"]=="ACPT"){ echo "selected";}?>>REG  Accepted</option>
                            <option value="CNCL" <?php if($_GET["os"]=="CNCL"){ echo "selected";}?>>REG Cancelled</option>
                            <?php }else{ ?>
                            <option selected value="">REG Status</option>
                            <option value="VRFY">REG Verifying</option>
                            <option value="ACPT">REG Accepted</option>
                            <option value="CNCL">REG Cancelled</option>
                            <?php } ?>
                        </select>
                    </div>
                    <div class="col-auto">
                        ...
                        <button type="reset" class="btn btn-danger"
                            onclick="javascript: window.location='admin_Reg_list.php'">Clear</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <?php
            if(isset($_GET["search"])){
                ...
                INNER JOIN tournament s ON orh.s_id = s.s_id WHERE c.c_type LIKE '%{$_GET['ut']}%' 
                AND orh_orderstatus LIKE '%{$_GET['os']}%'".$cid_clause.$sid_clause." ORDER BY orh.orh_ordertime DESC;";
            }else{
                $query = "SELECT orh.orh_id,orh.orh_ordertime,c.c_firstname,c.c_lastname,orh.orh_orderstatus,p.p_amount,s.s_name,orh.t_id
                FROM mreg_status orh INNER JOIN player c ON orh.c_id = c.c_id INNER JOIN payment p ON p.p_id = orh.p_id INNER JOIN tournament s ON orh.s_id = s.s_id ORDER BY orh.orh_ordertime DESC;";
            }
            ...
            <table class="table rounded-5 table-light table-striped table-hover align-middle caption-top mb-3">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <!--<a href="admin_mreg_detail.php?orh_id=<?php echo $row["orh_id"]?>" class="btn btn-sm btn-primary">View</a>-->
                            <a href="admin_Reg_update.php?orh_id=<?php echo $row["orh_id"]?>" class="btn btn-sm btn-outline-success">Update Status</a>
        ...
        <div class="row">
            <div class="col m-2 p-2 bg-danger text-white rounded text-start">
                <i class="bi bi-x-circle ms-2"></i><span class="ms-2 mt-2">No Registration was found</span>
                <?php if(isset($_GET["search"])){ ?>
