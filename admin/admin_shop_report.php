...
<head>
    ...
    <title>tournamentnament Revenue Report | kctms</title>
</head>
...
<body class="d-flex flex-column h-100 bg-white">
    ...
    <?php
        ...
        FROM tournamentnament WHERE s_id = {$s_id} LIMIT 0,1";
        $result = $mysqli -> query($query);
        $tournamentnament_row = $result -> fetch_array();
    ?>

    <div class="container px-5 py-4" id="tournamentnament-body">
        <a class="nav nav-item text-decoration-none text-muted mb-2" href="#" onclick="history.back();">
            <i class="bi bi-arrow-left-square me-2"></i>Go back
        </a>
        <div class="container row row-cols-6 row-cols-md-12 g-5 pt-4 mb-4" id="tournamentnament-header">
            <div class="rounded-25 col-6 col-md-4" id="tournamentnament-img" style="
                    background: url(
                        <?php
                            if(is_null($tournamentnament_row["s_pic"])){echo "'../img/default.png'";}
                            else{echo "'../img/{$tournamentnament_row['s_pic']}'";}
                        ?> 
                    ) center; height: 225px;
            ...
            <div class="col-6 col-md-8">
                <h1 class="display-5 strong"><?php echo $tournamentnament_row["s_name"];?></h1>
                <ul class="list-unstyled">
                    
                    <li class=""><?php echo $tournamentnament_row["s_location"];?></li>
                    
                    <li class="">Telephone number: <?php echo "(+91) ".$tournamentnament_row["s_phoneno"];?></li>
                </ul>
                <a class="btn btn-sm btn-outline-secondary" href="admin_tournamentnament_pwd.php?s_id=<?php echo $s_id?>">
                    <i class="bi bi-key"></i>
                    Change password
                </a>
                <a class="btn btn-sm btn-primary mt-2 mt-md-0" href="admin_tournamentnament_edit.php?s_id=<?php echo $s_id?>">
                    <i class="bi bi-pencil-square"></i>
                    Update tournamentnament profile
                </a>
                <a class="btn btn-sm btn-danger mt-2 mt-md-0" href="admin_tournamentnament_delete.php?s_id=<?php echo $s_id?>">
                    <i class="bi bi-trash"></i>
                    Delete this tournamentnament
                </a>
            </div>
        </div>
            ...
            switch($rev_mode){
                ...
                case 5: 
                        if(isset($_GET["start_date"])&&(isset($_GET["end_date"]))){
                            ...
                            header("location: tournamentnament_report_select.php"); exit(1);
                        }
                        break;
                default: header("location: tournamentnament_report_select.php"); exit(1);
            }
        ...
        <div class="container">
            <h3 class="border-top pt-3 my-2">
                <a class="text-decoration-none link-secondary"
                    href="admin_tournamentnament_detail.php?s_id=<?php echo $s_id?>">Menus</a>
                <span class="text-secondary">/</span>
                <a class="nav-item text-decoration-none link-secondary" href="admin_tournamentnament_order.php?s_id=<?php echo $s_id?>">Orders</a></span>
                <span class="text-secondary">/</span>
                <a class="nav-item text-decoration-none link-success"
                    href="admin_tournamentnament_revenue.php?s_id=<?php echo $s_id?>">Revenue</a></span>
            </h3>
                ...
                <div class="col">
                    <div class="card border-secondary">
                        <div class="card-body">
                            <h5 class="card-title">
                                <?php
                                    $query = "SELECT SUM(ord.ord_amount*ord.ord_buyprice) AS rev FROM mreg_status orh INNER JOIN mreg_detail ord ON orh.orh_id = ord.orh_id
                ...
                <div class="col">
                    <div class="card border-secondary">
                        <div class="card-body">
                            <h5 class="card-title">
                                <?php
                                    $query = "SELECT COUNT(*) AS cnt FROM mreg_statusorh 
                                    WHERE orh.s_id = {$s_id} AND orh_orderstatus = 'FNSH' AND (DATE(orh_finishedtime) BETWEEN DATE('{$start_date}') AND DATE('{$end_date}'));";
                                    $result = $mysqli -> query($query) -> fetch_array();
                ...
                <div class="col">
                    <div class="card border-secondary">
                        <div class="card-body">
                            <h5 class="card-title">
                                <?php
                                    $query = "SELECT COUNT(DISTINCT orh.c_id) AS cnt FROM mreg_status orh 
                                    WHERE orh.s_id = {$s_id} AND orh_orderstatus = 'FNSH' AND (DATE(orh_finishedtime) BETWEEN DATE('{$start_date}') AND DATE('{$end_date}'));";
                                    $result = $mysqli -> query($query) -> fetch_array();
                                    if(is_null($result["cnt"])){echo "0 players";} else{echo $result["cnt"]." players";}
                                ?>
                            </h5>
                            <p class="card-text small">Number of players</p>
                        </div>
                    </div>
                </div>
                <div class="col">
                ...
                <div class="col">
                    <div class="card border-secondary">
                        <div class="card-body">
                            <h5 class="card-title">
                                <?php
                                    $query = "SELECT SUM(ord.ord_amount) AS amt FROM mreg_status orh INNER JOIN mreg_detail ord ON orh.orh_id = ord.orh_id
                                    WHERE orh.s_id = {$s_id} AND orh_orderstatus = 'FNSH' AND (DATE(orh_finishedtime) BETWEEN DATE('{$start_date}') AND DATE('{$end_date}'));";
                                    $result = $mysqli -> query($query) -> fetch_array();
                ...
                <div class="col">
                    <div class="card border-secondary">
                        <div class="card-body">
                            <h5 class="card-title">
                                <?php
                                    $query = "SELECT f.f_name,SUM(ord.ord_amount) AS amt FROM mreg_status orh INNER JOIN mreg_detail ord ON orh.orh_id = ord.orh_id INNER JOIN matches f ON ord.f_id = f.f_id
                ...
                <div class="col">
                    <div class="card border-secondary">
                        <div class="card-body">
                            <h5 class="card-title">
                                <?php
                                    $query = "SELECT HOUR(orh_ordertime) AS odh,COUNT(orh_id) AS cnt FROM mreg_status orh
                ...
                <div class="col">
                    <div class="card border-secondary">
                        <div class="card-body">
                            <h5 class="card-title">
                                <?php
                                    $query = "SELECT HOUR(orh_finishedtime) AS odh,COUNT(orh_id) AS cnt FROM mreg_status orh
                                    WHERE s_id = {$s_id} AND orh_orderstatus = 'FNSH' AND (DATE(orh_finishedtime) BETWEEN DATE('{$start_date}') AND DATE('{$end_date}')) GROUP BY odh ORDER BY cnt DESC;";
                                    $result = $mysqli -> query($query);
                                    $num_rows = $result -> num_rows;
                                    if($num_rows == 0){echo "-";}
            ...
            <?php
                $query = "SELECT f.f_name,f.f_price,SUM(ord.ord_amount) AS amount,SUM(ord.ord_amount*ord.ord_buyprice) AS subtotal FROM mreg_status orh INNER JOIN mreg_detail ord ON orh.orh_id = ord.orh_id INNER JOIN matcheshes f ON ord.f_id = f.f_id
                WHERE orh.s_id = {$s_id} AND orh_orderstatus = 'FNSH' AND (DATE(orh_finishedtime) BETWEEN DATE('{$start_date}') AND DATE('{$end_date}'))
