...
<head>
    <?php 
        ...
        if(isset($_POST["rst_confirm"])){
            ...
            if($newpwd != $newcfpwd){
                ...
                $query = "UPDATE player SET c_pwd = '{$newpwd}' WHERE c_id = {$c_id}";
                $result = $mysqli -> query($query);
                if($result){
                    header("location: admin_player_detail.php?c_id={$c_id}&up_pwd=1");
                }else{
                    header("location: admin_player_detail.php?c_id={$c_id}&up_pwd=0");
                }
            }
        }
    ...
    <title>Update player password | kctms</title>
</head>
...
<body class="d-flex flex-column h-100">
    ...
    <div class="container form-signin mt-auto w-50">
        ...
        <form method="POST" action="admin_player_pwd.php" class="form-floating">
            <h2 class="mt-4 mb-3 fw-normal text-bold"><i class="bi bi-key me-2"></i>Update player Password</h2>
            ...
            <button class="w-100 btn btn-success my-3" name="rst_confirm" type="submit" onclick="return confirm('Do you want to update this player password?');" >Update Password</button>
        </form>
    </div>
