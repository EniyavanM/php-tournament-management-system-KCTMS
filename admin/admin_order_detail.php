...
<body class="d-flex flex-column h-100">
    ...
    <div class="container px-5 py-4" id="cart-body">
        <div class="row my-4 border-bottom">
            ...
            <?php
                $orh_id = $_GET["orh_id"];
                $orh_query = "SELECT * FROM mreg_statusWHERE orh_id = {$orh_id}";
                    ...
                    <ul class="list-unstyled fw-light">
                        ...
                        <li class="list-item">From
                            <a class="text-decoration-none link-primary"
                                href="admin_tournament_detail.php?c_id=<?php echo $orh_arr['s_id']?>">
                                <?php
                                $tournament_query = "SELECT s_name FROM tournament WHERE s_id = {$orh_arr['s_id']};";
                                $tournament_arr = $mysqli -> query($tournament_query) -> fetch_array();
                                echo $tournament_arr["s_name"];
                            ?>
                            </a>
                        </li>
                        <li class="list-item">Order of
                            <a class="text-decoration-none link-primary"
                                href="admin_player_detail.php?c_id=<?php echo $orh_arr['c_id']?>">
                                <?php
                                $cust_query = "SELECT c_firstname,c_lastname FROM player WHERE c_id = {$orh_arr['c_id']};";
            ...
            <div class="col">
                <div class="row row-cols-1">
                    ...
                    <div class="col row row-cols-1 row-cols-md-2 border-bottom">
                        <?php 
                            $ord_query = "SELECT f.f_id,f.f_name,f.f_pic,ord.ord_amount,ord.ord_buyprice,ord_note FROM mreg_detail ord INNER JOIN matches f ON ord.f_id = f.f_id WHERE ord.orh_id = {$orh_id}";
                        ...
                        <div class="col">
                            <ul class="list-group">
                                <a class="text-decoration-none"
                                    href="admin_matches_detail.php?f_id=<?php echo $ord_row["f_id"]?>">
                                    <li
