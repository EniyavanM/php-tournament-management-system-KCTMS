...
<head>
    <?php 
        ...
        if(isset($_POST["add_confirm"])){
            ...
            $insert_query = "INSERT INTO matches (f_name,f_price,s_id) 
            ...
            if(!empty($_FILES["f_pic"]["name"]) && $insert_result){
                ...
                if(move_uploaded_file($_FILES["f_pic"]["tmp_name"],SITE_ROOT.$target_file)){
                    $insert_query = "UPDATE matches SET f_pic = '{$target_newfilename}' WHERE f_id = {$f_id};";
                    $insert_result = $mysqli -> query($insert_query);
                }else{
                    $insert_result = false;
                }
            }
            if($insert_result){header("location: admin_matches_list.php?add_fdt=1");}
            else{header("location: admin_matches_list.php?add_fdt=0");}
            exit(1);
        }
        include('../head.php');
    ?>
...
<body class="d-flex flex-column h-100">
    ...
    <div class="container form-signin mt-auto w-50">
        ...
        <form method="POST" action="admin_match_add.php" class="form-floating" enctype="multipart/form-data">
