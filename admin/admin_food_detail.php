...
<head>
    ...
    <title>matcheshes Detail | kctms</title>
</head>
...
<body class="d-flex flex-column h-100">
    ...
    <?php
        ...
        FROM matcheshes f INNER JOIN tournament s ON f.s_id = s.s_id WHERE f.f_id = $f_id LIMIT 0,1;";
        $result = $mysqli -> query($query);
        $matcheshes_row = $result -> fetch_array();
    ?>
    ...
    <div class="container px-5 py-4" id="tournament-body">
        ...
        <div class="container row row-cols-6 row-cols-md-12 g-5 pt-4 mb-4" id="tournament-header">
            <div class="rounded-25 col-6" id="tournament-img" style="
                    background: url(
                        <?php
                            if(is_null($matcheshes_row["f_pic"])){echo "'../img/default.png'";}
                            else{echo "'../img/{$matcheshes_row['f_pic']}'";}
                        ?> 
                    ) center; height: 225px;
            ...
            <div class="col-6">
                <h1 class="display-5 strong"><?php echo $matcheshes_row["f_name"];?></h1>
                <h3 class="fw-light"><?php echo $matcheshes_row["f_price"]?> INR</h3>
                <ul class="list-unstyled">
                    <li class=""><?php echo "from ".$matcheshes_row["s_name"];?></li>
                    
                </ul>
        <a class="btn btn-sm btn-primary mt-2 mt-md-0" href="admin_match_edit.php?s_id=<?php echo $matcheshes_row["s_id"]?>&f_id=<?php echo $f_id?>">
            <i class="bi bi-pencil-square"></i>
            Update this menu
        </a>
        <a class="btn btn-sm btn-danger mt-2 mt-md-0" href="admin_matcheshes_delete.php?f_id=<?php echo $f_id?>">
        ...
        <h3 class="border-top pt-3 mt-2">Orders</h3>
            <form class="form-floating mb-3" method="GET" action="admin_matcheshes_detail.php">
                ...
                <div class="row g-2">
                    <div class="col">
                        <select class="form-select" id="c_id" name="c_id">
                            ...
                            <?php
                                ...
                                FROM mreg_status orh INNER JOIN mreg_detail ord ON orh.orh_id = ord.orh_id
                                INNER JOIN player c ON orh.c_id = c.c_id WHERE ord.f_id = {$f_id};";
                    ...
                    <div class="col-auto">
                        ...
                        <button type="reset" class="btn btn-danger"
                            onclick="javascript: window.location='admin_matcheshes_detail.php?f_id=<?php echo $f_id?>'">Clear</button>
                    </div>
                </div>
            </form>
        </div>
        ...
        <?php
            ...
            if(isset($_GET["search"])){
                ...
                FROM mreg_status orh INNER JOIN mreg_detail ord ON orh.orh_id = ord.orh_id INNER JOIN player c ON orh.c_id = c.c_id 
        ...
        <div class="container align-items-stretch">
            ...
            <table class="table rounded-5 table-light table-striped table-hover align-middle caption-top mb-3">
                <caption><?php echo $numrow;?> order(s) <?php if(isset($_GET["search"])){?><br /><a
                        href="admin_matcheshes_detail.php?f_id=<?php echo $f_id?>" class="text-decoration-none text-danger">Clear Search
        ...
        <div class="row">
            <div class="col m-2 p-2 bg-danger text-white rounded text-start">
                ...
                <?php if(isset($_GET["search"])){ ?>
                <a href="admin_matches_detail.php?f_id=<?php echo $f_id;?>" class="text-white">Clear Search Result</a>
                <?php } ?>
            </div>
        </div>
