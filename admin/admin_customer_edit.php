...
<head>
    <?php 
        ...
        if(isset($_POST["upd_confirm"])){
            ...
            $query = "UPDATE playerET c_firstname = '{$firstname}', c_lastname = '{$lastname}', c_email = '{$email}', c_gender = '{$gender}', c_type = '{$type}' WHERE c_id = {$c_id}";
            $result = $mysqli -> query($query);
            if($result){
                header("location: admin_playerist.php?up_prf=1");
            }else{
                header("location: admin_playerist.php?up_prf=0");
            }
            exit(1);
        }
        include('../head.php');
...
<body class="d-flex flex-column h-100">
    ...
    <div class="container form-signin mt-auto w-50">
        ...
        <?php 
            //Select playerecord from database
            $c_id = $_GET["c_id"];
            $query = "SELECT c_firstname,c_lastname,c_email,c_gender,c_type FROM playerHERE c_id = {$c_id} LIMIT 0,1";
            $result = $mysqli ->query($query);
            $row = $result -> fetch_array();
        ?>
        <form method="POST" action="admin_playerdit.php" class="form-floating">
