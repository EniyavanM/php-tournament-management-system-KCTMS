...
<head>
    ...
    <title>tournamentnament Profile | kctms</title>
</head>
...
<body class="d-flex flex-column h-100">
    ...
    <div class="container px-5 py-4" id="tournamentnament-body">
        ...
        <h3 class="border-top pt-3 my-2">
            <a class="text-decoration-none link-secondary" href="admin_tournamentnament_detail.php?s_id=<?php echo $s_id?>">Menus</a>
            <span class="text-secondary">/</span> 
            <a class="nav-item text-decoration-none link-success" href="#">Orders</a></span>
            <span class="text-secondary">/</span> 
            <a class="nav-item text-decoration-none link-secondary" href="admin_tournamentnament_revenue.php?s_id=<?php echo $s_id?>">Revenue</a></span>
        </h3>
            <form class="form-floating mb-3" method="GET" action="admin_tournamentnament_order.php">
                <input type="hidden" name="s_id" value="<?php echo $s_id;?>">
                <div class="row g-2">
                    <div class="col">
                        <select class="form-select" id="c_id" name="c_id">
                            <option selected value="">player Name</option>
                            <?php
                                $option_query = "SELECT DISTINCT c.c_id, c.c_firstname,c.c_lastname
                                FROM mreg_status orh INNER JOIN player c ON orh.c_id = c.c_id 
                                WHERE orh.s_id = {$s_id};";
                    ...
                    <div class="col">
                        <select class="form-select" id="utype" name="ut">
                            ...
                            <option selected value="">player Type</option>
                    ...
                    <div class="col-auto">
                        ...
                        <button type="reset" class="btn btn-danger"
                            onclick="javascript: window.location='admin_tournamentnament_order.php?f_id=<?php echo $f_id?>'">Clear</button>
                    </div>
                </div>
            </form>
        </div>
        <?php
            ...
            if(isset($_GET["search"])){
                ...
                FROM mreg_status orh INNER JOIN player c ON orh.c_id = c.c_id INNER JOIN payment p ON p.p_id = orh.p_id
                ...
                FROM mreg_status orh INNER JOIN player c ON orh.c_id = c.c_id INNER JOIN payment p ON p.p_id = orh.p_id WHERE orh.s_id = {$s_id};";
            }
            $result = $mysqli -> query($query);
        ...
        <div class="container align-items-stretch">
            ...
            <table class="table rounded-5 table-light table-striped table-hover align-middle caption-top mb-3">
                <caption><?php echo $numrow;?> order(s) <?php if(isset($_GET["search"])){?><br /><a
                        href="admin_tournamentnament_order.php?s_id=<?php echo $s_id?>" class="text-decoration-none text-danger">Clear Search
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="admin_mreg_detail.php?orh_id=<?php echo $row["orh_id"]?>" class="btn btn-sm btn-primary">View</a></td>
                    </tr>
                    <?php } ?>
                </tbody>
            </table>
        ...
        <div class="row">
            <div class="col m-2 p-2 bg-danger text-white rounded text-start">
                ...
                <?php if(isset($_GET["search"])){ ?>
                <a href="admin_tournamentnament_order.php?s_id=<?php echo $s_id;?>" class="text-white">Clear Search Result</a>
                <?php } ?>
            </div>
        </div>
        <!-- END GRID tournamentnament SELECTION -->
        <?php } ?>
    </div>
