...
<head>
    <?php 
        ...
        if(isset($_POST["upd_confirm"])){
            ...
            $query = "UPDATE player SET c_firstname = '{$firstname}', c_lastname = '{$lastname}', c_email = '{$email}', c_gender = '{$gender}', c_type = '{$type}' WHERE c_id = {$_SESSION['cid']}";
            $result = $mysqli -> query($query);
            if($result){
                $_SESSION["firstname"] = $firstname;
                $_SESSION["lastname"] = $lastname;
                header("location: player_profile.php?up_prf=1");
            }else{
                header("location: player_profile.php?up_prf=0");
            }
            exit(1);
        }
        include('head.php');
    ...
    <title>Update profile | kctmsitle>
</head>
...
<body class="d-flex flex-column h-100">
    ...
    <div class="container form-signin mt-auto w-50">
        ...
        <?php 
            ...
            $query = "SELECT c_firstname,c_lastname,c_email,c_gender,c_type FROM player WHERE c_id = {$_SESSION['cid']} LIMIT 0,1";
            $result = $mysqli ->query($query);
            $row = $result -> fetch_array();
        ?>
        <form method="POST" action="ply_update_profile.php" class="form-floating">
