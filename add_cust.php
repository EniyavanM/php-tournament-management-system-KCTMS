<?php
    //For inserting new player to database
    include('conn_db.php');
    $pwd = $_POST["pwd"];
    $cfpwd = $_POST["cfpwd"];
    if($pwd != $cfpwd){
        ...
        $query = "SELECT c_username FROM player WHERE c_username = '$username';";
        ...
        $query = "SELECT c_email FROM player WHERE c_email = '$email';";
        $result = $mysqli -> query($query);
        if($result -> num_rows >= 1){
            ?>
            <script>
        ...
        $query = "INSERT INTO player (c_username,c_pwd,c_firstname,c_lastname,c_email,c_gender,c_type)
