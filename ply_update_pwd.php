...
<head>
    <?php 
        ...
        if(isset($_POST["rst_confirm"])){
            ...
            if($newpwd != $newcfpwd){
                ...
                $query = "SELECT c_pwd FROM playerHERE c_id = {$_SESSION['cid']} LIMIT 0,1";
                $result = $mysqli -> query($query);
                $row = $result -> fetch_array();
                if($oldpwd == $row["c_pwd"]){
                    $query = "UPDATE playerET c_pwd = '{$newpwd}' WHERE c_id = {$_SESSION['cid']}";
                    $result = $mysqli -> query($query);
                    if($result){
                        header("location: player_profile.php?up_pwd=1");
                    }else{
                        header("location: player_profile.php?up_pwd=0");
                    }
                }else{
                    ?>
                    <script>
    ...
    <title>Update password | kctms</title>
</head>
...
<body class="d-flex flex-column h-100">
    ...
    <div class="container form-signin mt-auto w-50">
        ...
        <form method="POST" action="ply_update_pwd.php" class="form-floating">
