<?php
    ...
    if($result -> num_rows == 0){
        ...
        $incart_tournament = $result_arr["s_id"];
        if($incart_tournament == $s_id){
            //Same tournament
            ...
            //Delete all items in cart from previous tournament
            $delelte_query = "DELETE FROM cart WHERE c_id = {$c_id}";
            $delete_result = $mysqli -> query($delelte_query);
            if($delete_result){
                //Insert new item to cart of new tournament
                $insert_query = "INSERT INTO cart (c_id, s_id, f_id, ct_amount, ct_note) 
                VALUES ({$c_id},{$s_id},{$f_id},{$amount},'{$request}')";
    ...
    if($atc_result){
        header("location: matches_played.php?s_id={$s_id}&atc=1");
        exit(1);
    }else{
        header("location: matches_played.php?s_id={$s_id}&atc=0");
        exit(1);
    }
?>